...
// No fetch, account, token, or server is used on native platforms.
export function createLocalApi(repo: LocalRepository): ApiClient {
  return async <T>(path: string, options: Parameters<ApiClient>[1] = {}): Promise<T> => {
    ...
    switch (`${method} ${url.pathname}`) {
      ...
      case 'GET /api/foods/search': value = await repo.searchFoods(q('q')); break;
      case 'GET /api/foods/barcode': value = await repo.lookupBarcode(q('code')); break;
