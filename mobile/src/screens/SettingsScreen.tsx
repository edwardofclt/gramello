...
function GoalsEditor({ goals }: { goals: Goals }) {
  ...
  async function save() {
    ...
    saveLock.current = true; setSaving(true); setError(null); setSaved(false);
    try { setDraft(await api<Goals>('/api/goals', { method: 'PUT', body: draft })); setSaved(true); setEditing(null); }
    catch (error) { setError(errorMessage(error)); }
    finally { saveLock.current = false; setSaving(false); }
  }
  return <>
    <Card><View style={styles.row}><Target color={colors.mint} size={24} /><Text style={styles.heading}>Daily goals</Text></View><Text style={styles.muted}>Changing calories preserves your macro split. Changing a macro updates your calorie target.</Text>
      {(['calories', 'protein', 'carbs', 'fat'] as const).map(key => <View key={key} style={{ gap: 5 }}>
        ...
        {key !== 'calories' && <Text style={[styles.muted, { color: colors.mint }]}>{macroPercent(draft, key).toFixed(1)}% of energy</Text>}
      </View>)}
    ...
    {saved && <Text accessibilityRole="alert" style={{ color: colors.mint, textAlign: 'center' }}>Daily goals saved. Your web diary uses these too.</Text>}
    <Action busy={saving} disabled={!valid} onPress={() => void save()}>Save daily goals</Action>
  </>;
}

export function SettingsScreen() {
  const { api, name, email, signOut, busy } = useSession();
  ...
  return <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
    <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      <View><Text style={styles.eyebrow}>MAKE IT YOURS</Text><Text style={styles.title}>Settings</Text><Text style={styles.muted}>Your goals and account.</Text></View>
