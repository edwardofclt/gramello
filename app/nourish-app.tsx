...
import { Activity, CalendarDays, ChevronLeft, ChevronRight, Flame, LayoutDashboard, Loader2, Minus, Plus, ScanBarcode, Search, Settings2, Sparkles, Target, Trash2, TrendingUp, Utensils, X } from "lucide-react";
...
export default function NourishApp({ user }: { user: AuthUser }){
  ...
  useEffect(()=>{if(!addOpen||scanning||selected)return;if(query.trim().length<2){setResults([]);return}const abort=new AbortController();const timer=setTimeout(async()=>{setSearching(true);try{const r=await authFetch(`/api/foods/search?q=${encodeURIComponent(query)}`,{signal:abort.signal});const d=await r.json() as { error?: string; foods: Food[] };if(!r.ok)throw new Error(d.error);setResults(d.foods??[])}catch(e){if((e as Error).name!=="AbortError")toast.error("Food search is unavailable")}finally{setSearching(false)}},350);return()=>{clearTimeout(timer);abort.abort()}},[query,authFetch,addOpen,scanning,selected]);
  ...
  const lookupBarcode=useCallback(async(code:string,signal:AbortSignal)=>{
    ...
    if(!response.ok)throw new Error(data.error||"Barcode lookup is unavailable. Try again.");
    return data.food;
  },[authFetch]);
  const selectBarcodeFood=useCallback((food:Food)=>{setSelected(food);setScanning(false);setQuantity(1);setUnit("serving")},[]);
